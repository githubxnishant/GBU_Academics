...
const Dashboard = () => {
    ...
    const [searchedSem, setSearchedSem] = useState('');
    ...
    const submitHandle = async () => {
        ...
        try {
            if(subCode == '') {
                ...
                const duplicateSub = await axios.get(`${server}/checkduplicate/${dept}`, { params: { subCode } });
                if (duplicateSub.data.success) {
                    ...
                    if(subName == '') {
                        ...
                        await axios.post(`${server}/addsubject/${dept}`, {
    ...
    const searchHandle = async () => {
        try {
            const dept = activeTab.toLowerCase();
            let result = await axios.get(`${server}/searchsubject/${dept}`, {
    ...
    const handleUpdateClick = async () => {
        const dept = activeTab.toLowerCase();
        const newLink = prompt("Enter the new document link:", searchedDoc.docLink);
        if (newLink) {
            try {
                const response = await axios.put(`${server}/updatetable/${dept}`, {
                    subCode: searchedSubject.subCode,
                    subYear: searchedYear,
                    subSem: searchedSem,
                    docLink: newLink
                });
                if (response.data.success) {
                    alert("Document link updated successfully!");
                    setSearchedDoc(prevDoc => ({
                        ...prevDoc,
                        docLink: newLink
                    }));
                } else {
                ...
                alert("Error updating document link.");
            }
        }
    };  
    ...
    const handleDeleteClick = async () => {
        ...
        if (!confirmDelete) return;
            try {
                const response = await axios.put(`${server}/deletetable/${dept}`, {
                    subCode: searchedSubject.subCode,
                    subYear: searchedYear,
                    subSem: searchedSem,
                    docLink: ' '
                ...
                if (response.data.success) {
                    alert("Document link deleted successfully!");
                    setSearchedDoc(prevDoc => ({
                        ...prevDoc,
                        [`year${searchedYear}`]: {
                            ...prevDoc[`year${searchedYear}`],
                            [searchedSem]: " " ,
                        },
                        docLink: ' '
                    }));
                } else {
                    alert("Failed to delete document link.");
                }
            } catch (err) {
                console.error("Error deleting document link:", err);
                alert("Error deleting document link.");
            }
    }
    ...
    useEffect(() => {
        const fetchAdmin = async () => {
            try {
                const { data } = await axios.get(`${server}/dashboard`, {
    ...
    useEffect(() => {
        const fetchAllSubject = async () => {
            try {
                const dept = activeTab.toLowerCase();
                const response = await axios.get(`${server}/viewsubjects/${dept}`);
                                ...
                                <div className={`py-1 text-base ${activeTab == 'CSE' ? 'w-full' : 'hidden' }`}>
                                    ...
                                    <div className="w-full flex justify-between gap-3 mb-5">
                                        <div className="h-12 w-2/3 border bg-white px-5 flex justify-start items-center">
                                            ...
                                            <input 
                                            value={ searchSubject.subCode }
                                            onChange={(e) => {setSearchSubject(e.target.value)}}
                                            className="border w-20 border-black px-3 rounded-sm" maxLength={5} placeholder="Search Subject Code"/>
                                            <input value={searchedSubject.length != 0 ? searchedSubject.subName : '' } disabled className={`cursor-not-allowed w-80 border border-black px-3 ml-3 rounded-sm`} rel="noopener noreferrer" placeholder="Subject Name" />
                                    ...
                                    <div className={`h-auto border overflow-scroll w-full ${viewAllSubjects ? 'hidden' : 'w-full' }`}>
                                        <table className="w-full relative h-auto text-center">
                                            <thead>
                                            ...
                                            <tbody className="h-16 overflow-auto">
                                                <tr>
                                                    <td>
                                                        {searchedSubject.length != 0 ? <select className="w-full px-3 rounded" value={searchedYear} onChange={(e) => {setSearchedYear(e.target.value)}
                                                        }>
                                                            <option>Year</option>
                                                    ...
                                                    <td>
                                                        {searchedYear ? <select className="w-full px-3 rounded" value={searchedSem} onChange={(e) => {setSearchedSem(e.target.value)}}>
                                                            <option>Sem</option>
                                                    ...
                                                    <td className="mx-3">
                                                        {searchedSem ? <input  value={searchedDoc.docLink} onChange={setSearchedDoc} className="w-full px-3 rounded" disabled placeholder="Doc Link"/> : null }
                                                    </td>
                                                    <td>
                                                        {searchedSem ? 
                                                        <div>
                                                            <a href={searchedDoc.docLink} target="_blank"><button className="w-28 border mr-5 bg-blue-300 text-black px-4 py-1 rounded transition duration-150 hover:border-blue-600 hover:font-medium">View</button></a>
                                                            <button onClick={handleUpdateClick} className="w-28 border mr-5 bg-emerald-400 text-black px-4 py-1 rounded transition duration-150 hover:border-emerald-600 hover:font-medium">Update</button>
                                                            <button onClick={handleDeleteClick} className="w-28 border bg-red-400 px-4 py-1 rounded transition duration-150 hover:border-black hover:font-medium">Remove</button>
                                                        </div> : null}
                                                    </td>
                                                </tr>
                                            </tbody>
